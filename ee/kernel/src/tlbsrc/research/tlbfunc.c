...
static void Copy(void *dest, const void *src, unsigned int length){
	__asm volatile(	"li $v1, 0x5A\n"
			"syscall\n"
			"jr $ra\n"
			"nop\n");
}
...
static void *GetEntryAddress(int syscall){
	__asm volatile(	"li $v1, 0x5B\n"
			"syscall\n"
			"jr $ra\n"
			"nop\n");
}

static void setup(int syscall, void *function){
	__asm volatile(	"li $v1, 0x74\n"
			"syscall\n"
			"jr $ra\n"
			"nop\n");
}
...
void InitTLBFunctions(void){
	...
	Copy((void*)0x80075000, tlbsrc, 0x330);
	FlushCache(0);
	FlushCache(2);
...
void _SetTLBEntry(unsigned int index, unsigned int PageMask, unsigned int EntryHi, unsigned int EntryLo0, unsigned int EntryLo1){
	__asm volatile(	"li $v1, 0x56\n"
			"syscall\n"
			"jr $ra\n"
			"nop\n");
}

void iSetTLBEntry(unsigned int index, unsigned int PageMask, unsigned int EntryHi, unsigned int EntryLo0, unsigned int EntryLo1){
	__asm volatile(	"li $v1, -0x56\n"
			"syscall\n"
			"jr $ra\n"
			"nop\n");
...
static int InitTLB32MB(void){
	...
	__asm volatile(	"mtc0 $zero, Wired\n"
			"sync.p\n");
