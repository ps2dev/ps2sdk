...
#ifdef F_fflush
__attribute__((weak)) int mcFlush(int fd)
{
	return 0;
}

__attribute__((weak)) int mcSync(int mode, int *cmd, int *result)
{
	return 0;
}
...
size_t fwrite(const void *buf, size_t r, size_t n, FILE *stream)
{
	...
	switch (stream->type) {
		case STD_IOBUF_TYPE_GS:
			/* write to stdout. */
			for (i = 0, len = (r * n); i < len; ++i)
				putchar((int)((char *)buf)[i]);
			ret = r;
			break;
		case STD_IOBUF_TYPE_STDOUTHOST:
