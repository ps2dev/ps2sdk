...
u8 sceCdda_st_buf[64] ALIGNED(64);
#endif
...
void sceCdAlignReadBuffer(struct _cdvd_read_data *data)
{
	...
	sceCdGenericCallbackFunction((void*)&sceCdCallbackNum);
}
#endif
...
int sceCdRead(u32 lbn, u32 sectors, void *buf, sceCdRMode * mode)
{
	...
	if (sceCdNCmdDiskReady() == SCECdNotReady)
		return 0;
	if (sceCdCheckNCmd(CD_NCMD_READ) == 0)
		return 0;
	...
	sceCdCallbackNum = CD_NCMD_READ;
	cbSema = 1;

	if (SifCallRpc(&clientNCmd, CD_NCMD_READ, SIF_RPC_M_NOWAIT, readData, 24, 0, 0,
				(void *) sceCdAlignReadBuffer, _rd_intr_data) < 0) {
		sceCdCallbackNum = 0;
		cbSema = 0;
...
int sceCdReadDVDV(u32 lbn, u32 nsectors, void *buf, sceCdRMode *rm)
{
	if (sceCdNCmdDiskReady() == SCECdNotReady)
		return 0;
	if (sceCdCheckNCmd(CD_NCMD_DVDREAD) == 0)
		return 0;
	...
	sceCdCallbackNum = CD_NCMD_DVDREAD;
	cbSema = 1;

	if (SifCallRpc(&clientNCmd, CD_NCMD_DVDREAD, SIF_RPC_M_NOWAIT, readData, 24,
				NULL, 0, (void *)sceCdAlignReadBuffer, _rd_intr_data) < 0) {	//The Sony function was broken here: It doesn't set a callback, which will cause the library to enter a deadlocked state.
		sceCdCallbackNum = 0;
		cbSema = 0;
		SignalSema(nCmdSemaId);
		return 0;
...
int sceCdReadCDDA(u32 lbn, u32 nsectors, void *buf, sceCdRMode *rm)
{
	...
	if (sceCdNCmdDiskReady() == SCECdNotReady)
		return 0;
	if (sceCdCheckNCmd(CD_NCMD_CDDAREAD) == 0)
		return 0;
	...
	sceCdCallbackNum = CD_NCMD_CDDAREAD;
	cbSema = 1;

	if (SifCallRpc(&clientNCmd, CD_NCMD_CDDAREAD, SIF_RPC_M_NOWAIT, readData, 24,
				NULL, 0, (void *)sceCdAlignReadBuffer, _rd_intr_data) < 0) {
		sceCdCallbackNum = 0;
		cbSema = 0;
		SignalSema(nCmdSemaId);
		return 0;
...
int sceCdGetToc(u8 * toc)
{
	...
	if (sceCdCheckNCmd(CD_NCMD_GETTOC) == 0)
		return 0;
...
int sceCdSeek(u32 lbn)
{
	if (sceCdNCmdDiskReady() == SCECdNotReady)
		return 0;
	if (sceCdCheckNCmd(CD_NCMD_SEEK) == 0)
		return 0;

	seekSector = lbn;

	sceCdCallbackNum = CD_NCMD_SEEK;
	cbSema = 1;
	if (SifCallRpc(&clientNCmd, CD_NCMD_SEEK, SIF_RPC_M_NOWAIT, &seekSector, 4, 0, 0, sceCdGenericCallbackFunction, (void*)&sceCdCallbackNum) <
	    0) {
		sceCdCallbackNum = 0;
		cbSema = 0;
		SignalSema(nCmdSemaId);
...
int sceCdStandby(void)
{
	if (sceCdNCmdDiskReady() == SCECdNotReady)
		return 0;
	if (sceCdCheckNCmd(CD_NCMD_STANDBY) == 0)
		return 0;

	sceCdCallbackNum = CD_NCMD_STANDBY;
	cbSema = 1;
	if (SifCallRpc(&clientNCmd, CD_NCMD_STANDBY, SIF_RPC_M_NOWAIT, 0, 0, 0, 0, sceCdGenericCallbackFunction, (void*)&sceCdCallbackNum) < 0) {
		sceCdCallbackNum = 0;
		cbSema = 0;
		SignalSema(nCmdSemaId);
		return 0;
...
int sceCdStop(void)
{
	if (sceCdNCmdDiskReady() == SCECdNotReady)
		return 0;
	if (sceCdCheckNCmd(CD_NCMD_STOP) == 0)
		return 0;

	sceCdCallbackNum = CD_NCMD_STOP;
	cbSema = 1;
	if (SifCallRpc(&clientNCmd, CD_NCMD_STOP, SIF_RPC_M_NOWAIT, 0, 0, 0, 0, sceCdGenericCallbackFunction, (void*)&sceCdCallbackNum) < 0) {
		sceCdCallbackNum = 0;
		cbSema = 0;
		SignalSema(nCmdSemaId);
		return 0;
...
int sceCdPause(void)
{
	if (sceCdNCmdDiskReady() == SCECdNotReady)
		return 0;
	if (sceCdCheckNCmd(CD_NCMD_PAUSE) == 0)
		return 0;

	sceCdCallbackNum = CD_NCMD_PAUSE;
	cbSema = 1;
	if (SifCallRpc(&clientNCmd, CD_NCMD_PAUSE, SIF_RPC_M_NOWAIT, 0, 0, 0, 0, sceCdGenericCallbackFunction, (void*)&sceCdCallbackNum) < 0) {
		sceCdCallbackNum = 0;
		cbSema = 0;
		SignalSema(nCmdSemaId);
		return 0;
...
int sceCdApplyNCmd(u8 cmdNum, const void *inBuff, u16 inBuffSize, void *outBuff, u16 outBuffSize)
{
	if (sceCdNCmdDiskReady() == SCECdNotReady)
		return 0;
	if (sceCdCheckNCmd(CD_NCMD_NCMD) == 0)
		return 0;
...
int sceCdReadIOPMem(u32 lbn, u32 sectors, void *buf, sceCdRMode * mode)
{
	if (sceCdNCmdDiskReady() == SCECdNotReady)
		return 0;
	if (sceCdCheckNCmd(CD_NCMD_READIOPMEM) == 0)
		return 0;
	...
	sceCdCallbackNum = CD_NCMD_READIOPMEM;
	cbSema = 1;
	if (SifCallRpc(&clientNCmd, CD_NCMD_READIOPMEM, SIF_RPC_M_NOWAIT, readData, 24, 0, 0, sceCdGenericCallbackFunction, (void*)&sceCdCallbackNum) < 0) {
		sceCdCallbackNum = 0;
		cbSema = 0;
		SignalSema(nCmdSemaId);
		return 0;
...
int sceCdNCmdDiskReady(void)
{
	if (sceCdCheckNCmd(CD_NCMD_DISKREADY) == 0)
		return 0;
...
int sceCdReadChain(sceCdRChain * readChain, sceCdRMode * mode)
{
	...
	if (sceCdNCmdDiskReady() == SCECdNotReady)
		return 0;
	if (sceCdCheckNCmd(CD_NCMD_READCHAIN) == 0)
		return 0;
	...
	sceCdCallbackNum = CD_NCMD_READCHAIN;
	cbSema = 1;
	if (SifCallRpc
	    (&clientNCmd, CD_NCMD_READCHAIN, SIF_RPC_M_NOWAIT, readChainData, 788, 0, 0, sceCdGenericCallbackFunction,
	     (void*)&sceCdCallbackNum) < 0) {
		sceCdCallbackNum = 0;
		cbSema = 0;
		SignalSema(nCmdSemaId);
		return 0;
...
{
	if (sceCdCallbackNum == CD_NCMD_READ) {
...
int sceCdCddaStream(u32 lbn, u32 nsectors, void *buf, CdvdStCmd_t cmd, sceCdRMode *rm)
{
	...
	if (cmd == CDVD_ST_CMD_INIT)
		readStreamData[2] = (u32)sceCdda_st_buf;

	*(u32 *)sceCdda_st_buf = 0;
...
int sceCdCheckNCmd(int cmd)
{
	...
	while (1) {
		if (SifBindRpc(&clientNCmd, CD_SERVER_NCMD, 0) < 0) {
			if (sceCdDebug > 0)
				printf("LibsceCdvd bind err N CMD\n");
		}
...
int sceCdReadKey(unsigned char arg1, unsigned char arg2, unsigned int command, unsigned char *key)
{
	...
	if(sceCdCheckNCmd(CD_NCMD_READ_KEY)==0) return 0;
