...
{
    uint16_t* bf  = buf;
    uint32_t poly = 0, polyIn, di; //Initializing poly to 0 AND using it as initializer in the loop for polyIn results in the compiler forcing it always to 0 in the loop (so don't use variables as initializers - i.e. u32 polyIn = poly; ).
    int i;
    len /= 2; //to u16
    for (i = 0; i < len; i++) {
