	...
	if ( is_mg_elf )
	{
		...
		if ( SetLoadfileCallbacks_fnc )
		{
			...
			if ( CheckKelfPath_fnc && CheckKelfPath_fnc(filename, &v23, &v24) )
			{
				...
				{
					result =
						loadfile_functions.load_kelf_from_card(&allocate_info, &flhs, v23, v24, result_out, result_module_out);
				}
			}
			else if ( loadfile_functions.load_kelf_from_disk )
			{
				result = loadfile_functions.load_kelf_from_disk(&allocate_info, &flhs, result_out, result_module_out);
			}
		}
	}
