...
#define IOCTL_GETLBA 0xDEADBEEF /* Return the LBA of the opened file. */
...
static void fillStat(fio_stat_t *stat, fat_dir *fatdir)
{
...
static int nameSignature;
static int removalTime;
static int removalResult;
...
//---------------------------------------------------------------------------
static fs_rec* fs_findFileSlotByName(const char* name) {
	int i;
	for (i = 0; i < MAX_FILES; i++) {
...
//---------------------------------------------------------------------------
static int getNameSignature(const char *name) {
	int ret;
	int i;
	ret = 0;
...
int fs_ioctl(iop_file_t *fd, unsigned long request, void *data)
{
	...
	switch (request) {
		...
		case IOCTL_RENAME:  //Request to rename opened file/folder
			...
			break;
		case IOCTL_GETLBA:
			ret = fat_cluster2sector(&fatd->partBpb, ((fs_rec *)fd->privdata)->fatdir.chain[0].cluster);
			break;
		default:
			ret = -EIO;
	}
