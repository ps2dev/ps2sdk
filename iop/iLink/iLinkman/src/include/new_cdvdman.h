...
typedef struct {
	...
	unsigned int flag;       /* Bits 0-7: ISO9660 file type flag, Bits 8-31 are reserved */
} adv_cd_file_t;
...
typedef struct {
	unsigned int sectorLoc;	/* sector location to start reading from */
	unsigned int numSectors;	/* number of sectors to read */
	unsigned int buffer;	/* buffer address to read to ( bit0: 0=EE, 1=IOP ) */
...
int sceCdReadDVDV(unsigned int lsn,unsigned int sectors,void *buf,cd_read_mode_t *mode);  /* SP193: Corrected "int"s to "unsigned int"s in prototype */
...
int sceCdApplySCmd(unsigned char cmd, void *in, unsigned int in_size, void *out);
...
int sceCdReadKey(int arg1, int arg2, unsigned int lsn, char *key);
...
int sceCdReadCdda(unsigned int lsn, unsigned int sectors, void *buf, cd_read_mode_t *mode);
#define I_sceCdReadCdda DECLARE_IMPORT(40, sceCdReadCdda)
int sceCdReadConsoleID(u64 *id, unsigned int *stat);                     /* SP193: Changed "unsigned int *res, int *stat" to "u64 *id, unsigned int *stat" */
#define I_sceCdReadConsoleID DECLARE_IMPORT(41, sceCdReadConsoleID)

/* SP193: Added export #43 */
int sceCdMV(unsigned int *mv,unsigned int *stat);
...
int sceCdForbidDVDP(unsigned int *stat);
#define I_sceCdForbidDVDP DECLARE_IMPORT(52, sceCdForbidDVDP)
int sceCdReadSUBQ(char *buf,unsigned int *stat);
...
int sceCdStRead(unsigned int size,void *buf,unsigned int mode,unsigned int *err);
...
int sceCdRead0(unsigned int lsn, unsigned int sectors, void *buf, cd_read_mode_t *mode, int csec, void *cb);
#define I_sceCdRead0 DECLARE_IMPORT(62, sceCdRead0)
int sceCdRV(unsigned int lsn, unsigned int sectors, void *buf, cd_read_mode_t *mode, int csec, void *cb);
#define I_sceCdRV DECLARE_IMPORT(63, sceCdRV)
int sceCdRM(char *m,unsigned int *stat);
...
int sceCdForbidRead(unsigned int *);
...
int sceCdReadFull(unsigned int lsn,unsigned int sectors,void *buf,cd_read_mode_t *mode);
...
int sceCdReadModelID(unsigned int *id);                                  /* SP193: Note! DNAS IOPRP images only! */
...
int sceCdApplySCmd2(unsigned char cmd,void *wdata,unsigned int wdlen,void *rdata);
#define I_sceCdApplySCmd2 DECLARE_IMPORT(112, sceCdApplySCmd2)
int sceCdReadIOPm(unsigned int lsn,unsigned int sectors,void *buf,cd_read_mode_t *mode);
